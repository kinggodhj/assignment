...
parser.add_argument('--path', type=str, default='./premodel/')
...
writer = SummaryWriter('./runs/%sPre%s%s%s%s'%(BATCH_SIZE, EMB_SIZE, NUM_ENCODER_LAYERS, NUM_EPOCHS, MAX_LEN))
...
if __name__ == "__main__":
    source_file = "./data/bt.train_x.0.txt"
    target_file = "./data/bt.train_y.0.txt"
    ...
    for epoch in range(1, NUM_EPOCHS+1):
        ...
        if val_loss < best:
            torch.save(transformer.state_dict(), './allmodel/%smodel%s%s%s%s.pkt'%(BATCH_SIZE, epoch, EMB_SIZE, NHEAD, NUM_ENCODER_LAYERS))
